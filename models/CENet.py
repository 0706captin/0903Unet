...
class PSPModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = [torch.upsample(input=stage(feats), size=(h, w), mode='bilinear') for stage in self.stages] + [feats]
...
class SPPblock(nn.Module):
    ...
    def forward(self, x):
        ...
        self.layer1 = torch.upsample(self.conv(self.pool1(x)), size=(h, w), mode='bilinear')
        self.layer2 = torch.upsample(self.conv(self.pool2(x)), size=(h, w), mode='bilinear')
        self.layer3 = torch.upsample(self.conv(self.pool3(x)), size=(h, w), mode='bilinear')
        self.layer4 = torch.upsample(self.conv(self.pool4(x)), size=(h, w), mode='bilinear')
