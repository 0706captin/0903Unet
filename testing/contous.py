...
# print(at)
# print(at[0])
# print(torch.max(at[0]))
# print(torch.max(at[1]))
...
def save_hd(hd, name):
    ...
    dataframe.to_csv('./results/s2s_hd.csv')


def savecontours(image_root, gt_root, pred_root, name):
    """
    输入是三个图片的路径
    """
    ...
    for each_conts in areas:
        if each_conts > 9000:
            ...
            contours_pre = Leave_target(contours_pre)
            contours_pre = strip(contours_pre)
            cv2.fillPoly(im1_gray, [contours_pre], color=(0, 0, 0))
            # cv2.drawContours(im1_gray, [contours_pre], 0, (0, 255, 0), 5, lineType=cv2.LINE_8)
    ...
    plt.imshow(a)
    plt.axis('off')
    save_demo = '/home/huangjq/PycharmCode/cxj_Code/B4_attUnetv4/testing/0903demo'
    mkfile(save_demo)
    a.save(os.path.join(save_demo, name))
    plt.show()

    return contours_gt, contours_pre


root = '/home/huangjq/PycharmCode/cxj_Code/B4_attUnetv4/results/images/0903v1/k0'
...
for i in range(len(name_list)):
    contours_gt, contours_pre = savecontours(input_root, label_root, ouput_root, name_list[i])
    print(name_list[i])
    print(contours_pre.shape)
    draw(contours_pre)
    plt.cla()
    plt.close('all')
